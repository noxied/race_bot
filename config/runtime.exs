...
if config_env() == :prod do
  ...
  # Where the live session snapshot is persisted so the bot keeps its memory
  # across restarts. Defaults to a file next to the SQLite database (same
  # mounted volume). Disabled in demo mode.
  unless demo_mode_enabled do
    config :f1_bot,
      session_snapshot_path:
        System.get_env("SESSION_SNAPSHOT_PATH") ||
          Path.join(Path.dirname(database_path), "session_snapshot.bin")
  end
